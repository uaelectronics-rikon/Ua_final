...
if (GMAIL_USER && GMAIL_PASS) {
  ...
  const transportConfig = {
    host: "smtp.gmail.com",
    port: 465,                   // ✅ FIX: Port 465 SSL — port 587 is BLOCKED on Render free tier
    secure: true,                // ✅ FIX: must be true for port 465
    family: 4,                   // ✅ Force IPv4 — Render IPv6 is unreachable (ENETUNREACH)
    ...
    // ✅ FIX: pool:false — pooled connections go stale on Render ephemeral containers
    pool: false,
...
async function sendEmailWithRetry(mailOptions, retries = 5, delay = 3000) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      // ✅ FIX: removed verify() from send path — it was blocking on Render DNS warmup
      // and consuming retry attempts before sendMail() was ever called
      ...
      if (attempt < retries) {
        ...
        // Reinitialize transporter before retry
        transporter = null;
