...
dns.setDefaultResultOrder("ipv4first"); // ✅ FIX: Force IPv4 globally — Render IPv6 is unreachable
...
if (GMAIL_USER && GMAIL_PASS) {
  ...
  const transportConfig = {
    ...
    family: 4,                   // Force IPv4 (backup — dns.setDefaultResultOrder is primary)
    ...
    socketTimeout: 45000,
    pool: false,                 // ✅ FIX: no pooling — stale sockets on Render
...
async function sendEmailWithRetry(mailOptions, retries = 5, delay = 3000) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      // ✅ FIX: verify() removed — was blocking on Render and killing retries
      ...
      // EAUTH = wrong credentials — retrying will never help
      if (error.code === 'EAUTH') {
